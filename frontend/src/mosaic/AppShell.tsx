...
const base =
   "block flex items-center justify-between rounded-3xl border px-6 py-5 min-h-16 mx-2 " +
  "transition-all duration-200 hover:-translate-y-0.5 " +
  "shadow-[0_6px_18px_rgba(15,23,42,0.06)] hover:shadow-[0_10px_24px_rgba(15,23,42,0.10)] " +
  "ring-1 ring-slate-900/5 ring-offset-2 ring-offset-slate-50";

   const idle = "bg-white border-slate-200 text-slate-800 hover:bg-slate-50";
  const active = "bg-grey border-violet-300 text-violet-700";
  const labelCls = `text-xl font-medium transition-all duration-300 ${props.expanded ? "opacity-100 w-auto" : "lg:opacity-0 lg:w-0 lg:overflow-hidden"}`;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className="h-6 w-6 flex items-center justify-center">{props.icon}</div>
        <span className={labelCls}>{props.label}</span>
      </div>
      {typeof props.badgeCount === "number" && props.badgeCount > 0 ? (
        <span className="min-w-7 h-7 px-2 inline-flex items-center justify-center rounded-full bg-red-500 text-white text-sm font-semibold">
          {props.badgeCount}
        </span>
      ) : null}
    </NavLink>
...
export default function AppShell({ sidebarOpen, setSidebarOpen, children }: Props) {
  ...
  return (
 <div className="flex h-screen overflow-hidden bg-slate-50 lg:gap-6 lg:px-6">
      <div
      ...
      <aside
        ...
        ref={sidebarRef}
        className={[
          "fixed z-50 left-0 top-0 h-screen lg:sticky lg:top-6 lg:z-auto",
          "w-72 lg:w-72",
          expanded ? "" : "lg:w-20",
     "shrink-0 bg-slate-50 border-slate-200 lg:rounded-2xl lg:shadow-sm lg:border lg:border-slate-200 lg:my-6",
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <NavLink to="/" end className="flex items-center gap-3" aria-label="Home">
              ...
              <span
                className={`font-bold text-2xl text-slate-900 transition-all duration-300 ${expanded ? "opacity-100 w-auto" : "lg:opacity-0 lg:w-0 lg:overflow-hidden"}`}
              >
                GoStock 
              </span>
            </NavLink>
 ...
 <nav className="flex flex-col gap-2 flex-1 px-4 lg:px-6 py-6 overflow-y-auto">
          <SideItem to="/home" label="Home" icon={<IconHome />} end expanded={expanded} />
          <SideItem to="/dashboard" label="Dashboard" icon={<IconGrid />} expanded={expanded} />
          <SideItem to="/wallet" label="Wallet" icon={<IconWallet />} expanded={expanded} />
          ...
          <SideItem to="/news" label="Portfolio" icon={<IconWallet />} expanded={expanded} />
        ...
        <div className="px-4 pb-4 space-y-2 border-t border-slate-100">
          ...
          <div className="hidden lg:block pt-2">
            ...
            >
              ...
              <span className={`text-sm font-medium transition-all duration-300 ${expanded ? "opacity-100 w-auto" : "lg:opacity-0 lg:w-0 lg:overflow-hidden"}`}>
                Collapse
              </span>
            </button>
          </div>
